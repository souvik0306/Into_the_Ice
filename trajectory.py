...
# Noise parameters
noise_amplitude = 0.01  # Further reduced noise amplitude for acceptable perturbations
collision_threshold = 0.1  # Minimum distance from the cave boundary before adjusting trajectory

# PID controller parameters
Kp = 1.0  # Proportional gain (slightly increased)
Ki = 0.01  # Integral gain (small to avoid integral windup)
Kd = 0.5  # Derivative gain (moderate to smooth oscillations)
...
for i in range(1, num_points):
    ...
    # Introduce acceptable random noise into both trajectories
    noise_x = noise_amplitude * np.random.randn()
    noise_y = noise_amplitude * np.random.randn()

    # Undamped trajectory (no control system)
    x_undamped[i] = np.clip(x_undamped[i-1] + vx_undamped + noise_x, left_thickness[i] + collision_threshold, right_thickness[i] - collision_threshold)
    y_undamped[i] = np.clip(y_undamped[i-1] + vy_undamped + noise_y, left_thickness[i] + collision_threshold, right_thickness[i] - collision_threshold)
    vx_undamped += noise_x
    vy_undamped += noise_y

    # Damped trajectory with PID control
    x_damped[i] = np.clip(x_damped[i-1] + vx_damped + noise_x, left_thickness[i] + collision_threshold, right_thickness[i] - collision_threshold)
    y_damped[i] = np.clip(y_damped[i-1] + vy_damped + noise_y, left_thickness[i] + collision_threshold, right_thickness[i] - collision_threshold)
