...
def run_simulation():
    ...
    left_thickness = np.zeros(num_points)  # Left boundary of the cave
    right_thickness = np.zeros(num_points)  # Right boundary of the cave

    # Robot's position marker for the thickness plot
    robot_position_x = []  # X position of the robot in the thickness plot

    # Generate random cave thickness around a cylinder
    base_radius = 1.5  # Average radius of the cave cylinder
    random_variation = 0.3  # Variation around the base radius for a natural look

    # Left and right thickness boundaries with random offsets at each depth level
    for i in range(num_points):
        offset = np.random.uniform(-random_variation, random_variation)
        left_thickness[i] = -(base_radius + offset)
        right_thickness[i] = base_radius + offset
    ...
    for i in range(1, num_points):
        ...
        # Determine the max safe radius within the cave boundaries
