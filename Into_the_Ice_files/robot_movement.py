...
def simulate_robot_movement(left_thickness, right_thickness, desired_x, desired_y):
    ...
    for i in range(1, NUM_POINTS):
        ...
        vx_damped, vy_damped, integral_x, integral_y = apply_PID_control(
            i, x_damped[i], y_damped[i], vx_damped, vy_damped, 0, 0, desired_x[i], desired_y[i]
        )
        ...
        error = np.sqrt((x_damped[i] - desired_x[i])**2 + (y_damped[i] - desired_y[i])**2)
        error_percentage.append(error / (np.sqrt(desired_x[i]**2 + desired_y[i]**2) + 1e-6) * 100)
