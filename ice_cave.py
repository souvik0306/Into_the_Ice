...
# Robot's proximity to walls
proximity_to_walls = []  # To store how close the robot is to the cave walls

# Update the simulation loop to track proximity to walls
for i in range(1, num_points):
    # Elliptical trajectory definition
    ellipse_x = a * np.cos(theta_values[i]) + noise_amplitude * np.random.randn()  # Ellipse with noise
    ellipse_y = b * np.sin(theta_values[i]) + noise_amplitude * np.random.randn()
    ...
    # Ensure the robot stays away from both ends (avoid collision with walls)
    x_damped[i] = np.clip(x_damped[i-1] + vx_damped + noise_x, left_thickness[i] + collision_margin, right_thickness[i] - collision_margin)
    y_damped[i] = np.clip(y_damped[i-1] + vy_damped + noise_y, left_thickness[i] + collision_margin, right_thickness[i] - collision_margin)
    vx_damped, vy_damped, integral_x, integral_y = apply_PID_control(
        i, theta_values[i], x_damped[i], y_damped[i], vx_damped, vy_damped, 0, 0, random_radius, ellipse_x, ellipse_y
    ...
    # Calculate how close the robot is to the walls
    proximity_left = x_damped[i] - left_thickness[i]  # Distance from left wall
    proximity_right = right_thickness[i] - x_damped[i]  # Distance from right wall
    proximity_to_walls.append(min(proximity_left, proximity_right))  # Track minimum proximity to any wall
...
def update(frame):
    ...
    line_proximity.set_data(range(frame), proximity_to_walls[:frame])

    return line_damped_3d, line_undamped_3d, line_damped_2d, line_undamped_2d, line_left_thickness, line_right_thickness, robot_marker, line_error, line_proximity
